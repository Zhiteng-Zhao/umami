...
export async function savePageView(website_id, session_id, url, referrer, username, syscode) {
  return runQuery(
...
export async function saveEvent(website_id, session_id, url, event_type, event_value, username) {
  return runQuery(
    prisma.event.create({
      data: {
        website: {
...
export function getWebsiteForStatistics(website_id, start_at, end_at) {
  return runQuery(
    prisma.pageview.findMany({
      where: {
...
export function getPageviewMetrics(website_id, start_at, end_at, field, table, filters = {}) {
  const params = [website_id, start_at, end_at];
  const { domain, url, username, syscode } = filters;
  ...
  if (username) {
    userFilter = `and username=$${params.length + 1}`;
    params.push(`${username}`);
  }

  if (syscode) {
    syscodeFilter = `and syscode=$${params.length + 1}`;
    params.push(`${syscode}`);
  }

  return rawQuery(
    `
    select ${field} x, count(*) y, syscode
    from ${table}
    where website_id=$1
    and created_at between $2 and $3
    ${domainFilter}
...
export function getPageviewCycle(website_id, start_at, end_at, url, username, syscode) {
  const params = [website_id, start_at, end_at];
  ...
  return rawQuery(
    `
    select username , url , count(url) x
    from event
    where website_id=$1
    and created_at between $2 and $3
    ${userFilter}
