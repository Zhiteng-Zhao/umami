...
export async function savePageView(website_id, session_id, url, referrer, username) {
  return runQuery(
...
export function getPageviewCycle(website_id, start_at, end_at, url, username) {
  const params = [website_id, start_at, end_at, url, username];

  let userFilter = '';
  if (username) {
    userFilter = `and username=$4`;
    params.push(`${username}`);
  }
